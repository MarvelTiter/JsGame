...
function calcProj(axis: Vector2, path: Vector2[]): ProjectionInfo {
    let v: Vector2 = path[0]
    let d: number, min: number, max: number, mip: Vector2, map: Vector2
    let pointOnAxis = false
...
function segDist(p1: ProjectionInfo, p2: ProjectionInfo, axis: Vector2): ProjectResult {
    if (p2.min > p1.max) {
        let dist = p2.min - p1.max
        let d = axis.copy().normalize().multi(dist)
        return {
